import { Injectable } from '@nestjs/common';
import { ConfigService } from '@nestjs/config';
import { I18nOptionsFactory, I18nOptionsWithoutResolvers } from 'nestjs-i18n';
import { join } from 'path';

/**
 * Service responsible for configuring the i18n module
 * Implements I18nOptionsFactory to provide i18n configuration options
 */
@Injectable()
export class I18nConfigService implements I18nOptionsFactory {
  constructor(private readonly configService: ConfigService) {}

  /**
   * Creates the i18n options configuration
   * @returns I18n configuration options with resolvers
   */
  createI18nOptions(): I18nOptionsWithoutResolvers {
    return {
      fallbackLanguage: this.configService.get<string>(
        'app.fallbackLanguage',
        'en',
      ),
      loaderOptions: {
        path: join(__dirname, '..', '..', '..', 'i18n'),
        watch: true,
      },
      typesOutputPath: join(process.cwd(), 'src/generated/i18n.generated.ts'),
      viewEngine: 'hbs',
    };
  }
}
